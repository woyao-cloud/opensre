...
- `search_github_code` 会自动把这个查询补成：
  - `你的查询 + repo:woyao-cloud/ollama-03-02-claude-prompt02-template`
...
当前代码里，`get_github_file_contents` 想自动可用，需要 `github.path` 存在。  
而这个路径来自告警里的：
...
1. 先用通用示例跑一次，确认 GitHub + OpenSearch 整条链能工作。
2. 再换成 `ollama-template-repo-alert.json`，看 planner 是否更偏向 GitHub 模板/配置调查。
...
对 `woyao-cloud/ollama-03-02-claude-prompt02-template` 这种目标仓库，  
OpenSRE 最应该避免的是“只用通用错误词做代码搜索”。
